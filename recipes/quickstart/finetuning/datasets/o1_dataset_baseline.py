...
class o1preview(torch.utils.data.Dataset):
    def __init__(
        ...
        def apply_prompt_template(sample):
            ...
            # 随机挑选一些答案充当中间的推理步骤
            num_submissions = random.randint(0, min(num_recycles, len(submissions)))
        ...
        self.dataset = self.dataset.map(apply_prompt_template, remove_columns=list(self.dataset.features))

        def tokenize_add_label(sample):
            input_ids = tokenizer.encode(tokenizer.bos_token + sample["input"], add_special_tokens=False)
            output_ids = tokenizer.encode(sample["output"] + tokenizer.eos_token, add_special_tokens=False)
        ...
        if tokenizer is not None:
            self.dataset = self.dataset.map(tokenize_add_label, remove_columns=list(dataset.features))
...
if __name__ == "__main__":
    split = "train"
    dataset = get_custom_dataset(dataset_config=None, tokenizer=None, split=split)
